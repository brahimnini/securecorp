...
def get_user(user_id):
    ...
    if user and is_owner_or_admin(user.id):
...
def get_ticket(ticket_id):
    ...
    if ticket and is_owner_or_admin(ticket.author_id):
        return jsonify({'id': ticket.id, 'title': ticket.title, 'content': ticket.content, 'author_id': ticket.author_id})
    return jsonify({'message': 'Accès non autorisé ou ticket non trouvé'}), 403
